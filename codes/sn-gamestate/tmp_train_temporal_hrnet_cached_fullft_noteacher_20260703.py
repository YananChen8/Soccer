...
def compute_losses(model, teacher_model, batch, device, use_amp, args):
    ...
    with torch.amp.autocast(device_type=device.type, dtype=torch.float16, enabled=use_amp):
        pred = model(win)
        teacher = None
        if args.teacher_weight > 0 and teacher_model is not None:
            with torch.no_grad():
        ...
        if args.teacher_weight > 0 and teacher is not None:
            ...
            teacher_raw = pred.float().sum() * 0.0
...
def main():
    ...
    if args.teacher_weight > 0:
        ...
        teacher_model = None
        print("TEACHER_DISABLED teacher_weight=0", flush=True)
