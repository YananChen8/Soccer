...
# ── Patched: parallelize the per-frame keypoint-extraction loop ───────────────
# generate_class_synthesis()/get_line_extremities() are pure-Python/OpenCV, CPU-bound,
# single-frame-at-a-time, and were the real bottleneck (not the GPU segmentation forward
# pass, not the gradient-descent optimization step): they ran serially on one core for
# every one of the ~7500 valid10 frames. Each frame's extraction is fully independent, so
# this is embarrassingly parallel -- dispatch across a persistent process pool instead.
_EXTRACT_POOL = None
...
def _get_extract_pool():
    ...
    if _EXTRACT_POOL is None:
        ...
        log.info(f"[tvcalib patch] starting keypoint-extraction process pool with {n} workers")
        _EXTRACT_POOL = ProcessPoolExecutor(max_workers=n)
    return _EXTRACT_POOL


def _extract_one(sem_line, fn_synthesis, fn_extremities):
    skeleton = fn_synthesis(sem_line)
    return fn_extremities(skeleton)
...
class TVCalib_Segmentation(ImageLevelModule):
    ...
    def process(self, batch: Any, detections: pd.DataFrame, metadatas: pd.DataFrame):
        ...
        futures = [
            pool.submit(_extract_one, sem_line, self.fn_generate_class_synthesis, self.fn_get_line_extremities)
            for sem_line in sem_lines
        ]
        keypoints_raw = [f.result() for f in futures]

        self._n_processed += len(sem_lines)
        log.info(f"[tvcalib patch] pitch keypoint extraction: {self._n_processed} frames done so far")
...
class TVCalib(ImageLevelModule):
    ...
    def __init__(self, image_width, image_height, lens_dist, optim_steps, batch_size, device, **kwargs):
        ...
        # Patched: log_per_step=True + tqdm enabled so the 2000-step optimization actually
        # reports progress (previously fully silent for the entire optim run -- looked stalled).
        self.model = TVCalibModule(
                        ...
                        device,
                        log_per_step=True,
                        tqdm_kwqargs={"disable": False, "desc": "tvcalib optim"},
                    )
        self._n_batches = 0
    ...
    def process(self, batch: Any, detections: pd.DataFrame, metadatas: pd.DataFrame):
        ...
        self._n_batches += 1
        log.info(f"[tvcalib patch] starting optim batch #{self._n_batches} ({_batch_size} frames, 2000 steps)")
        per_sample_loss, cam, _ = self.model.self_optim_batch(batch)
        log.info(f"[tvcalib patch] finished optim batch #{self._n_batches}")
