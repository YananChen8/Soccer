...
def masked_mse(pred, gt, mask, fg_weight):
    """NBJW-style masked L2: nn.MSELoss(reduction='none'), then (loss * mask).mean()."""
    pred = pred[:, : gt.size(1)].float()
    channel_mask = mask[:, :, None, None]
    mse = (pred - gt).square()
    if fg_weight <= 0:
        return (mse * channel_mask).mean()
    weight = torch.ones_like(gt)
    if fg_weight > 0:
        fg = torch.zeros_like(gt)
        fg[:, :-1] = (gt[:, :-1] > 0.1).float()
        weight = weight + fg_weight * fg
    return (mse * channel_mask * weight).mean()
...
def scaled_aux(raw, ref, ratio, max_scale):
    if ratio <= 0:
        return raw.new_zeros(())
    scale = ratio * ref.detach() / (raw.detach().abs() + 1e-8)
...
def train_batch(pending, model, optimizer, scaler, device, use_amp, args, step, totals, accum_idx):
    ...
    with torch.amp.autocast(device_type=device.type, dtype=torch.float16, enabled=use_amp):
        ...
        peak_term = scaled_aux(peak, heat, args.peak_ratio, args.aux_max_scale)
        motion_term = scaled_aux(motion, heat, args.motion_ratio, args.aux_max_scale)
        loss = heat + peak_term + motion_term
        loss_for_backward = loss / max(args.grad_accum_steps, 1)

    scaler.scale(loss_for_backward).backward()
    accum_idx += 1
    if accum_idx >= max(args.grad_accum_steps, 1):
    ...
    step_metrics = {
        ...
        "heat": float(heat.detach().item()),
        "peak": float(peak.detach().item()),
        "motion": float(motion.detach().item()),
        "peak_term": float(peak_term.detach().item()),
        "motion_term": float(motion_term.detach().item()),
        "fg": float(fg_mse(pred, gt, mask)),
        "frames": int(len(pending)),
    }
    totals["loss"] += step_metrics["loss"]
    totals["heat"] += step_metrics["heat"]
    totals["peak"] += step_metrics["peak"]
    totals["motion"] += step_metrics["motion"]
    totals["peak_term"] += step_metrics["peak_term"]
    totals["motion_term"] += step_metrics["motion_term"]
    totals["fg"] += step_metrics["fg"]
    totals["batches"] += 1
    totals["frames"] += len(pending)
    pending.clear()
    return step, accum_idx, step_metrics
...
def main():
    ...
    ap.add_argument("--peak-ratio", type=float, default=0.2)
    ap.add_argument("--motion-ratio", type=float, default=0.0)
    ap.add_argument("--aux-max-scale", type=float, default=200000.0)
    ap.add_argument("--grad-accum-steps", type=int, default=2)
    ap.add_argument("--grad-clip-norm", type=float, default=1.0)
    ap.add_argument("--shuffle-chunk-frames", type=int, default=30)
    ...
    def checkpoint_payload(epoch, steps):
        return {
            ...
            "loss_design": {
                "name": "official_masked_mse_scaled_aux",
                "heat": "NBJW MSELoss style: ((pred-target)^2 * mask).mean()",
                "peak_ratio": args.peak_ratio,
                "motion_ratio": args.motion_ratio,
                "aux_max_scale": args.aux_max_scale,
            ...
            "shuffle_chunk_frames": args.shuffle_chunk_frames,
            "split": args.split,
            "videos": video_names,
            "epoch": epoch,
            "steps": steps,
    ...
    print(
        ...
        f"window={args.window_size} batch={args.batch_size} grad_accum={args.grad_accum_steps} "
        f"peak_ratio={args.peak_ratio} motion_ratio={args.motion_ratio} fg_weight={args.fg_weight}",
        flush=True,
    )
    global_step = 0
    run_start = time.perf_counter()
    use_amp = device.type == "cuda"
    loss_csv_path = Path(args.out).with_name("step_losses.csv")
    loss_fields = [
        "epoch", "step", "optimizer_step", "frames", "loss", "heat", "peak",
        "motion", "peak_term", "motion_term", "fg", "hrnet_lr", "adapter_lr",
        "frames_per_second",
    ]
    loss_csv_file = loss_csv_path.open("w", newline="")
    loss_writer = csv.DictWriter(loss_csv_file, fieldnames=loss_fields)
    loss_writer.writeheader()
    loss_csv_file.flush()

    def write_step_loss(epoch_idx, metrics, opt_step):
        loss_writer.writerow({
            ...
            "peak_term": metrics["peak_term"],
            "motion_term": metrics["motion_term"],
            "fg": metrics["fg"],
            "hrnet_lr": optimizer.param_groups[0]["lr"],
            "adapter_lr": optimizer.param_groups[1]["lr"],
            "frames_per_second": totals["frames"] / max(time.perf_counter() - run_start, 1),
        })
    for epoch in range(args.epochs):
        chunks = []
        for video in video_names:
            frames = videos[video][: args.max_frames or None]
            size = max(args.shuffle_chunk_frames, args.window_size)
            for start in range(0, len(frames), size):
                chunks.append((video, frames[start : start + size]))
        random.shuffle(chunks)
        pending = []
        accum_idx = 0
        totals = {
            "loss": 0.0, "heat": 0.0, "peak": 0.0, "motion": 0.0,
            "peak_term": 0.0, "motion_term": 0.0, "fg": 0.0, "batches": 0, "frames": 0,
        }
        for video, frames in chunks:
            history = []
            for _image_id, stem in frames:
                ...
                if len(pending) >= args.batch_size:
                    before = global_step
                    global_step, accum_idx, step_metrics = train_batch(pending, model, optimizer, scaler, device, use_amp, args, global_step, totals, accum_idx)
                    write_step_loss(epoch, step_metrics, accum_idx == 0)
                    if global_step % 50 == 0:
                        loss_csv_file.flush()
                    if args.log_every and global_step != before and global_step % args.log_every == 0:
                        ...
                        print(
                            ...
                            f"peak_term={totals['peak_term']/denom:.6f} motion_term={totals['motion_term']/denom:.6f} "
        ...
        print(
            ...
            f"motion={totals['motion']/denom:.6f} peak_term={totals['peak_term']/denom:.6f} "
            f"motion_term={totals['motion_term']/denom:.6f} fg={totals['fg']/denom:.6f} saved={epoch_path}",
            flush=True,
        )
        if args.max_steps and global_step >= args.max_steps:
            break
